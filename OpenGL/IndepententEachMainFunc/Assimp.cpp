...
int main()
{
	...
	// ------------------------------------
	Shader lightShader("diffuse_tex_vertex.glsl", "utility_calcLight.glsl");

	Shader lampShader("light_vertex.glsl", "light_fragment.glsl");
	...
	unsigned char* data = stbi_load("container2.png", &width, &height, &nChannel, 0);
	GLenum format;
	if (nChannel == 1)
		format = GL_RED;
	else if (nChannel == 3)
	...
	data = stbi_load("container2_specular.png", &width, &height, &nChannel, 0);
	if (nChannel == 1)
		format = GL_RED;
	else if (nChannel == 3)
		format = GL_RGB;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightShader.setVec3("dirLight.direction", -0.2f, -0.1f, -0.3f);
		...
		lightShader.setFloat("pointLights[0].quadratice", 0.032);
		// point light 2
